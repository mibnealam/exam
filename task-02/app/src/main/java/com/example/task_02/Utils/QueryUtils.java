...
import com.example.task_02.Models.Phone;
...
/**
 * Helper methods related to requesting and receiving user data from api.
 */
public final class QueryUtils {
    ...
    /**
     * Query the api data set and return a list of {@link User} objects.
     */
    ...
    public static ArrayList<User> extractFeatureFromJson(String userJSON) {
        ...
        try {
            ...
            for (int i = 0; i < userArray.length(); i++) {
                ...
                // Get a single user at position i within the list of user
                JSONObject currentUser = userArray.getJSONObject(i);

                // For a given user, extract the JSONObject associated with the
                // key called "properties", which represents a list of all properties
                // for that user.
                JSONObject phonesJsonObj = currentUser.getJSONObject("phones");

                // Extract the value for the key called "firstName"
                long id = currentUser.getLong("id");

                // Extract the value for the key called "firstName"
                String firstName = currentUser.getString("firstName");

                // Extract the value for the key called "lastName"
                ...
                // Extract the value for the key called "lastName"
                int photo = currentUser.getInt("photo");

                // Extract the value for the key called "lastName"
                Phone phones = new Phone();
                phones.setHome(phonesJsonObj.getString("home"));
                phones.setMobile(phonesJsonObj.getString("mobile"));


                // Create a new {@link User} object with the firstName, lastName, phones,
                // and photo from the JSON response.
                User user = new User(id, firstName, lastName, phones, gender, photo);
